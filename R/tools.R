...
get_dim_value <- function(x, dimname) {
  ...
  if (dimname %in% c("nspecies", "n_species_max")) {
    list_species <- get_species_list(x)
...
attr_legend <- function(df) {
  ...
  if (length(this.units) == 2) {
    this.units <- ifelse(this.units == "", "-", this.units)
    return( 
      paste0(this.longname, " (", this.units,")") 
      )
  } else if (!is.null(this.units) && this.units != "") {
    return(
      paste0(this.longname, " (", this.units,")")
    )
  } else {
...
##' @author Remi Lemaire-Patin
##' 
##' @title List all variables names
##' 
##' @description List all variables names in a MuSICA
##'   \code{netcdf} output.
##' 
##' @param x a \code{ncdf4} object. Output generated by MuSICA
##' 
...
##' @author Remi Lemaire-Patin
##' 
##' @title Get simulated species list
##' 
##' @description This function retrieve the secies list from a \code{ncdf4}
##'  object, output of MuSICA
##' 
##' @param x a \code{ncdf4} object, output of MuSICA
##' @return
##' 
##' A \code{character vector}
##' @family getters
